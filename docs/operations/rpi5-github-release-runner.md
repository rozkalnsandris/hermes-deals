...
Only controlled JSON evidence generated by the root dispatcher is copied to the
runner artifact directory. Before upload, the dispatcher rejects non-regular
members, oversized evidence and common private-key, GitHub-token, database URL
and `PGPASSWORD` patterns.
