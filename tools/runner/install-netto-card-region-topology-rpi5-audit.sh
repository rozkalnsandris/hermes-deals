...
[[ $# -eq 2 ]] || fail "usage: sudo bash tools/runner/install-netto-card-region-topology-rpi5-audit.sh <merged-main-sha> <clean-detached-source-worktree>"
EXPECTED_SHA="$1"
SOURCE_REPO="$(readlink -f -- "$2")"
EXPECTED_SOURCE_REPO='/home/andris/hermes-deals-worktrees/netto-card-region-topology-audit-v1'
...
AUDIT_TOOL="$RUNTIME_ROOT/tools/netto_card_region_topology_audit.py"
...
CONFIG="$REGISTRY_DIR/netto-card-region-topology-audit-v1.conf"
...
SOURCE_AUDIT_TOOL="$SOURCE_REPO/tools/netto_card_region_topology_audit.py"
...
for relative in \
  ...
  tools/runner/install-netto-card-region-topology-rpi5-audit.sh; do
  git_source -C "$SOURCE_REPO" ls-files --error-unmatch "$relative" >/dev/null || fail "required source is not tracked: $relative"
done
for source in "$SOURCE_AUDIT_TOOL" "$SOURCE_OWNERSHIP_AUDIT_TOOL" "$SOURCE_BASE_REPLAY_TOOL" "$SOURCE_PARSER_TOOL" "$SOURCE_OWNERSHIP_TRUTH"; do
  [[ -f "$source" && ! -L "$source" ]] || fail "required source is missing or unsafe: $source"
done

/usr/bin/python3 - "$SOURCE_AUDIT_TOOL" "$SOURCE_OWNERSHIP_AUDIT_TOOL" "$SOURCE_BASE_REPLAY_TOOL" "$SOURCE_PARSER_TOOL" <<'PY'
...
TMPDIR_INSTALL="$(mktemp -d /tmp/hermes-deals-netto-topology-installer.XXXXXX)"
...
audit_name='netto-card-region-topology-audit-v1'
...
[[ $# -eq 2 ]] || fail "usage: hermes-deals-netto-card-region-topology-audit-dispatch <registered-commit-sha> <artifact-dir>"
EXPECTED_SHA="$1"
EXPORT_DIR="$2"
...
STAGING_ROOT='/home/andris/hermes-deals-runner-evidence'
[[ -f "$CONFIG" && ! -L "$CONFIG" ]] || fail "topology audit is not registered"
[[ "$(stat -c '%U:%G' "$CONFIG")" == 'root:root' ]] || fail "audit config ownership is invalid"
# shellcheck disable=SC1090
source "$CONFIG"
[[ "${audit_name:-}" == 'netto-card-region-topology-audit-v1' ]] || fail "audit config name mismatch"
[[ "${commit_sha:-}" == "$EXPECTED_SHA" ]] || fail "requested commit is not the registered audit commit"
[[ "${runtime_root:-}" == '/usr/local/libexec/hermes-deals-audits/netto-card-region-topology-audit-v1' ]] || fail "runtime root mismatch"
...
PYMUPDF_VERSION="$(runuser -u andris -- /usr/bin/env -i HOME=/home/andris USER=andris LOGNAME=andris PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 - <<'PY'
import pymupdf
print(pymupdf.pymupdf_version)
PY
)"
...
[[ "$EXPORT_DIR" == /home/github-runner/_work/_temp/hermes-deals-netto-card-region-topology-audit-* ]] || fail "artifact directory is outside runner temp allowlist"
...
[[ "$RUN_KEY" =~ ^hermes-deals-netto-card-region-topology-audit-[0-9]+-[0-9]+$ ]] || fail "artifact directory name is invalid"
...
runuser -u andris -- /usr/bin/env -i \
  ...
  /usr/bin/python3 "$audit_tool_path" \
    ...
    --output "$STAGING_DIR/card-region-topology-audit.json" \
    > "$LOG_FILE" 2>&1
AUDIT_RC=$?
set -e
...
if [[ $AUDIT_RC -eq 0 ]]; then
  /usr/bin/python3 - "$STAGING_DIR/card-region-topology-audit.json" <<'PY'
import json
from pathlib import Path
import sys
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if payload.get("cell_count") != 100:
    raise SystemExit("topology audit cell count mismatch")
if payload.get("classification_performed") is not False:
    raise SystemExit("topology audit classification boundary violated")
if payload.get("parser_behavior_changed") is not False:
    raise SystemExit("topology audit parser boundary violated")
if payload.get("review_only") is not True or payload.get("promotion_ready") is not False:
    raise SystemExit("topology audit promotion boundary violated")
for key in ("automatic_approval_enabled", "automatic_publish_enabled", "database_write_performed", "deployment_performed"):
    if payload.get(key) is not False:
        raise SystemExit(f"topology audit safety flag violated: {key}")
PY
fi
/usr/bin/python3 - "$STAGING_DIR/runtime-identity.json" "$EXPECTED_SHA" "$audit_tool_sha256" "$ownership_audit_tool_sha256" "$base_replay_tool_sha256" "$parser_tool_sha256" "$ownership_truth_sha256" "$n9_manifest_sha256" "$PYMUPDF_VERSION" <<'PY'
import json
from pathlib import Path
import sys
Path(sys.argv[1]).write_text(json.dumps({
    "audit": "netto-card-region-topology-audit-v1",
    "commit_sha": sys.argv[2],
    "audit_tool_sha256": sys.argv[3],
    "ownership_audit_tool_sha256": sys.argv[4],
    "base_replay_tool_sha256": sys.argv[5],
    "parser_tool_sha256": sys.argv[6],
    "ownership_truth_sha256": sys.argv[7],
    "n9_manifest_sha256": sys.argv[8],
    "pymupdf_version": sys.argv[9],
...
for name in audit-execution.log audit-exit-code.txt runtime-identity.json; do
  install -o github-runner -g github-runner -m 0600 "$STAGING_DIR/$name" "$EXPORT_DIR/audit-evidence/$name"
done
if [[ -f "$STAGING_DIR/card-region-topology-audit.json" ]]; then
  install -o github-runner -g github-runner -m 0600 "$STAGING_DIR/card-region-topology-audit.json" "$EXPORT_DIR/audit-evidence/card-region-topology-audit.json"
fi
(
