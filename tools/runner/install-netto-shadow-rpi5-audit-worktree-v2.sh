...
for command in chown find git readlink runuser stat; do
...
UNEXPECTED_OWNER="$(find "$WORKTREE_ADMIN" -xdev \! -user andris -print -quit)"
...
runuser -u andris -- /usr/bin/env \
  GIT_OPTIONAL_LOCKS=0 \
  git -C "$SOURCE_REPO" status --porcelain=v1 --untracked-files=all \
  | grep -q . && fail "dedicated worktree is not clean after installation"

INDEX_PATH="$(git -C "$SOURCE_REPO" rev-parse --git-path index)"
