...
[[ $# -eq 1 ]] || fail "usage: sudo bash tools/runner/install-aldi-visual-card-bridge-v2-dispatcher.sh <merged-commit-sha>"
EXPECTED_SHA="$1"
...
LIBEXEC='/usr/local/libexec/hermes-deals-audits/aldi-visual-card-bridge-v2'
...
SUDOERS='/etc/sudoers.d/hermes-deals-aldi-visual-card-bridge-v2'

declare -A source_files=(
  [diagnostic]='tools/aldi_visual_card_bridge_diagnostic_v2.py'
  ...
  [dispatcher]='tools/runner/aldi-visual-card-bridge-v2-dispatcher.sh'
)
...
tmp="$(mktemp -d /tmp/hermes-aldi-visual-card-bridge-v2-install.XXXXXX)"
cleanup() { rm -rf -- "$tmp"; }
trap cleanup EXIT
...
Defaults!/usr/local/sbin/hermes-deals-aldi-visual-card-bridge-v2 env_reset,secure_path=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
github-runner ALL=(root) NOPASSWD: /usr/local/sbin/hermes-deals-aldi-visual-card-bridge-v2
SUDOERS
...
install -o root -g root -m 0555 "$tmp/diagnostic" "$LIBEXEC/aldi_visual_card_bridge_diagnostic_v2.py"
...
diagnostic_sha="$(sha256sum "$LIBEXEC/aldi_visual_card_bridge_diagnostic_v2.py" | awk '{print $1}')"
...
conf_tmp="$(mktemp /etc/hermes-deals-audits.d/.aldi-visual-card-bridge-v2.conf.XXXXXX)"
cat > "$conf_tmp" <<CONF
registered_main_sha='$EXPECTED_SHA'
diagnostic_path='$LIBEXEC/aldi_visual_card_bridge_diagnostic_v2.py'
...
sudo -l -U github-runner | grep -Fq "$DISPATCHER" || fail "github-runner v2 sudo rule missing"
