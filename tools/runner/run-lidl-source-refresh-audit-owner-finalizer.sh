...
for command in gh git readlink sha256sum stat sudo; do
...
printf 'UTC=%s\nTARGET_SHA=%s\nPR_NUMBER=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$TARGET_SHA" "$PR_NUMBER"
printf 'PRIMARY_HEAD_BEFORE=%s\n' "$PRIMARY_HEAD_BEFORE"

gh auth status >/dev/null
PR_JSON="$(gh api "repos/$REPOSITORY/pulls/$PR_NUMBER")"
python3 - "$TARGET_SHA" <<'PY' <<<"$PR_JSON"
import json, sys
expected = sys.argv[1]
pr = json.load(sys.stdin)
if not pr.get('merged') or not pr.get('merged_at'):
    raise SystemExit('PR is not merged')
if pr.get('merge_commit_sha') != expected:
    raise SystemExit('PR merge SHA mismatch')
base = pr.get('base') or {}
if base.get('ref') != 'main' or (base.get('repo') or {}).get('full_name') != 'rozkalnsandris/hermes-deals':
    raise SystemExit('PR was not merged into repository main')
PY
...
sudo bash "$INSTALLER" "$TARGET_SHA" | tee /tmp/hermes-lidl-source-refresh-install.log
for marker in \
  'INSTALL_RESULT=PASS' \
  ...
  grep -Fxq "$marker" /tmp/hermes-lidl-source-refresh-install.log || fail "installer marker missing: $marker"
done
rm -f /tmp/hermes-lidl-source-refresh-install.log
...
printf 'PRIMARY_INVARIANCE=true\n'
