...
FINALIZER_VERSION='netto-object-card-graph-audit-reregister-owner-finalizer-v1'
...
EXPECTED_COMMON_DIR='/home/andris/hermes-deals/.git'

fail() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }

[[ $# -eq 0 ]] || fail 'usage: run-netto-object-card-graph-audit-reregister-owner-finalizer.sh'
...
LOG="/home/andris/netto-object-card-graph-audit-reregister-owner-finalizer-${stamp}.log"
INSTALL_LOG="$(mktemp /tmp/netto-object-card-graph-reregister-install.XXXXXX)"
SUDO_LIST_LOG="$(mktemp /tmp/netto-object-card-graph-reregister-sudo-list.XXXXXX)"
cleanup() { rm -f -- "$INSTALL_LOG" "$SUDO_LIST_LOG"; }
trap cleanup EXIT
exec > >(tee -a "$LOG") 2>&1

text_sha256() { printf '%s' "$1" | sha256sum | awk '{print $1}'; }

git_read() {
  local repo="$1"; shift
  local out err rc stderr_text
  out="$(mktemp /tmp/netto-object-card-graph-reregister-git-out.XXXXXX)"
  err="$(mktemp /tmp/netto-object-card-graph-reregister-git-err.XXXXXX)"
  set +e
...
snapshot_primary() {
  ...
  PRIMARY_INDEX="$(git_read "$PRIMARY" rev-parse --path-format=absolute --git-path index)"
  [[ -f "$PRIMARY_INDEX" && ! -L "$PRIMARY_INDEX" ]] || fail 'primary Git index is unsafe'
  PRIMARY_INDEX_SHA="$(sha256sum "$PRIMARY_INDEX" | awk '{print $1}')"
}

verify_primary_unchanged() {
  [[ "$(git_read "$PRIMARY" branch --show-current)" == "$PRIMARY_BRANCH" ]] || fail 'primary branch changed'
  [[ "$(git_read "$PRIMARY" rev-parse HEAD)" == "$PRIMARY_HEAD" ]] || fail 'primary HEAD changed'
  local status
  ...
  [[ "$(text_sha256 "$status")" == "$PRIMARY_STATUS_SHA" ]] || fail 'primary status digest changed'
  [[ "$(git_read "$PRIMARY" rev-parse --path-format=absolute --git-path index)" == "$PRIMARY_INDEX" ]] || fail 'primary index path changed'
  [[ "$(sha256sum "$PRIMARY_INDEX" | awk '{print $1}')" == "$PRIMARY_INDEX_SHA" ]] || fail 'primary Git index changed'
}

verify_worktree_identity() {
  [[ -d "$WORKTREE" && ! -L "$WORKTREE" ]] || fail 'audit worktree path is unsafe'
  [[ "$(stat -c '%U:%G' "$WORKTREE")" == andris:andris ]] || fail 'audit worktree ownership mismatch'
  [[ -z "$(git_read "$WORKTREE" branch --show-current)" ]] || fail 'audit worktree must be detached'
  [[ -z "$(git_read "$WORKTREE" status --porcelain=v1 --untracked-files=all)" ]] || fail 'audit worktree is dirty'
  local common origin
  common="$(git_read "$WORKTREE" rev-parse --git-common-dir)"
  case "$common" in
    /*) common="$(readlink -f -- "$common")" ;;
    *) common="$(readlink -f -- "$WORKTREE/$common")" ;;
  esac
  [[ "$common" == "$EXPECTED_COMMON_DIR" ]] || fail 'audit worktree is not attached to primary repository'
  ...
  case "$origin" in
    ...
    *) fail 'audit worktree origin is not allowlisted' ;;
  esac
}

config_value() {
  local key="$1"
  awk -F= -v wanted="$key" '$1 == wanted { value=$2; sub(/^\047/, "", value); sub(/\047$/, "", value); print value; found=1; exit } END { if (!found) exit 1 }' "$CONFIG"
}

printf '=== Netto #415 object-card graph audit re-registration owner finalizer ===\n'
printf 'FINALIZER_VERSION=%s\nSOURCE_PR=%s\nTARGET_SHA=%s\nLOG=%s\n' "$FINALIZER_VERSION" "$SOURCE_PR" "$TARGET_SHA" "$LOG"
...
[[ "$CI_OK" -ge 1 ]] || fail 'exact target SHA has no successful main-push CI'
...
if [[ -e "$WORKTREE" ]]; then
  verify_worktree_identity
  current_sha="$(git_read "$WORKTREE" rev-parse HEAD)"
...
verify_worktree_identity
[[ "$(git_read "$WORKTREE" rev-parse HEAD)" == "$TARGET_SHA" ]] || fail 'audit worktree HEAD mismatch after refresh'
verify_primary_unchanged

INSTALLER="$WORKTREE/$INSTALLER_REL"
[[ -f "$INSTALLER" && ! -L "$INSTALLER" ]] || fail 'reviewed installer is missing or unsafe'
set +e
sudo bash "$INSTALLER" "$TARGET_SHA" "$WORKTREE" 2>&1 | tee "$INSTALL_LOG"
INSTALL_RC=${PIPESTATUS[0]}
set -e
[[ "$INSTALL_RC" -eq 0 ]] || fail 'root trust installer failed'
for marker in 'INSTALL_RESULT=PASS' "REGISTERED_SHA=$TARGET_SHA" 'AUDIT_EXECUTED=false' 'DATABASE_WRITE=false' 'REVIEW_WRITE=false' 'APPROVAL_PUBLICATION=false' 'PRODUCTION_DEPLOY=false'; do
...
[[ "$(stat -c '%U:%G:%a' "$RUNTIME_ROOT")" == root:root:755 ]] || fail 'runtime root metadata mismatch'
[[ -f "$DISPATCHER" && ! -L "$DISPATCHER" && -x "$DISPATCHER" ]] || fail 'dispatcher is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$DISPATCHER")" == root:root:755 ]] || fail 'dispatcher metadata mismatch'
[[ -f "$CONFIG" && ! -L "$CONFIG" ]] || fail 'config is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$CONFIG")" == root:root:644 ]] || fail 'config metadata mismatch'
[[ -f "$SUDOERS" && ! -L "$SUDOERS" ]] || fail 'sudoers is missing or unsafe'
...
grep -Fq "$DISPATCHER" "$SUDO_LIST_LOG" || fail 'runner dispatcher authorization is missing'
if id -nG github-runner | tr ' ' '\n' | grep -Fxq docker; then fail 'github-runner unexpectedly belongs to Docker group'; fi
