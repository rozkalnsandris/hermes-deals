...
FINALIZER_VERSION='netto-missing-normal-price-audit-owner-finalizer-v1'
...
INSTALLER_REL='tools/runner/install-netto-missing-normal-price-rpi5-audit.sh'
...
[[ $# -eq 0 ]] || fail 'usage: run-netto-missing-normal-price-audit-owner-finalizer.sh'
...
LOG="/home/andris/netto-missing-normal-price-audit-owner-finalizer-${stamp}.log"
INSTALL_LOG="$(mktemp /tmp/netto-missing-normal-price-audit-install.XXXXXX)"
SUDO_LIST_LOG="$(mktemp /tmp/netto-missing-normal-price-audit-sudo-list.XXXXXX)"
exec > >(tee -a "$LOG") 2>&1
trap on_error ERR
cleanup() {
...
git_read() {
  ...
  out="$(mktemp /tmp/netto-missing-normal-price-git-out.XXXXXX)"
  err="$(mktemp /tmp/netto-missing-normal-price-git-err.XXXXXX)"
  set +e
...
printf '=== Netto #321 missing-normal-price audit owner finalizer ===\n'
printf 'UTC=%s\nFINALIZER_VERSION=%s\nSOURCE_PR=%s\nTARGET_SHA=%s\nLOG=%s\n' \
...
if [[ -e "$WORKTREE" ]]; then
  [[ -d "$WORKTREE" && ! -L "$WORKTREE" ]] || fail 'missing-normal-price audit worktree path is unsafe'
else
  install -d -m 0755 "$(dirname "$WORKTREE")"
  git -C "$PRIMARY" worktree add --detach "$WORKTREE" "$TARGET_SHA"
fi

[[ "$(git_read "$WORKTREE" rev-parse HEAD)" == "$TARGET_SHA" ]] || fail 'missing-normal-price audit worktree HEAD mismatch'
[[ -z "$(git_read "$WORKTREE" branch --show-current)" ]] || fail 'missing-normal-price audit worktree must be detached'
[[ -z "$(git_read "$WORKTREE" status --porcelain=v1 --untracked-files=all)" ]] || fail 'missing-normal-price audit worktree is dirty'
[[ "$(stat -c '%U:%G' "$WORKTREE")" == andris:andris ]] || fail 'missing-normal-price audit worktree ownership mismatch'
...
[[ "$COMMON_DIR" == '/home/andris/hermes-deals/.git' ]] || fail 'missing-normal-price audit source is not a primary-repository worktree'
...
case "$origin" in
  ...
  *) fail 'missing-normal-price audit worktree origin is not allowlisted' ;;
esac
verify_primary_unchanged

INSTALLER="$WORKTREE/$INSTALLER_REL"
[[ -f "$INSTALLER" && ! -L "$INSTALLER" ]] || fail 'reviewed missing-normal-price audit installer is missing or unsafe'
set +e
sudo bash "$INSTALLER" "$TARGET_SHA" "$WORKTREE" 2>&1 | tee "$INSTALL_LOG"
INSTALL_RC=${PIPESTATUS[0]}
set -e
[[ "$INSTALL_RC" -eq 0 ]] || fail 'missing-normal-price audit root trust installer failed'

grep -Fxq 'INSTALL_RESULT=PASS' "$INSTALL_LOG" || fail 'installer PASS marker mismatch'
grep -Fxq "REGISTERED_SHA=$TARGET_SHA" "$INSTALL_LOG" || fail 'installer registered SHA mismatch'
grep -Fxq 'PYMUPDF_VERSION=1.28.0' "$INSTALL_LOG" || fail 'installer PyMuPDF marker mismatch'
...
for member in \
  "$RUNTIME_ROOT/tools/netto_missing_normal_price_audit.py" \
  "$RUNTIME_ROOT/tools/netto_visual_geometry_corpus_replay.py" \
  "$RUNTIME_ROOT/tools/netto_visual_geometry_shadow.py" \
  "$RUNTIME_ROOT/backend/tests/fixtures/netto/n10_full_visual_review_v1.json"; do
...
[[ -f "$CONFIG" && ! -L "$CONFIG" ]] || fail 'installed missing-normal-price audit config is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$CONFIG")" == 'root:root:644' ]] || fail 'installed missing-normal-price audit config metadata mismatch'
[[ -f "$SUDOERS" && ! -L "$SUDOERS" ]] || fail 'installed missing-normal-price audit sudoers is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$SUDOERS")" == 'root:root:440' ]] || fail 'installed missing-normal-price audit sudoers metadata mismatch'
sudo --non-interactive visudo -cf "$SUDOERS" >/dev/null

[[ "$(config_value audit_name)" == 'netto-missing-normal-price-audit-v1' ]] || fail 'registered audit name mismatch'
[[ "$(config_value commit_sha)" == "$TARGET_SHA" ]] || fail 'registered missing-normal-price commit mismatch'
...
[[ "$(config_value n10_ledger_sha256)" == "$EXPECTED_N10_SHA" ]] || fail 'registered N10 SHA mismatch'
[[ "$(config_value corpus_root)" == "$CORPUS_ROOT" ]] || fail 'registered corpus root mismatch'
...
[[ "$(sha256sum "$N10_LEDGER" | awk '{print $1}')" == "$EXPECTED_N10_SHA" ]] || fail 'N10 ledger SHA256 mismatch'

PYMUPDF_VERSION="$(/usr/bin/env -i HOME=/home/andris USER=andris LOGNAME=andris PATH=/usr/bin:/bin PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 - <<'PY'
import pymupdf
...
grep -Fq "$DISPATCHER" "$SUDO_LIST_LOG" || fail 'runner missing-normal-price dispatcher authorization is missing'
...
printf 'NEXT_GITHUB_ACTION=apply audit:netto-missing-normal-price-v1 to merged PR #438\n'
