...
[[ $# -eq 1 ]] || fail 'usage: sudo bash tools/runner/install-lidl-source-sync-bridge.sh <merged-main-sha>'
EXPECTED_SHA="$1"
REPO='/home/andris/hermes-deals'
SOURCE_REL='tools/runner/hermes-deals-lidl-source-sync-dispatch'
SOURCE="$REPO/$SOURCE_REL"
DISPATCHER='/usr/local/sbin/hermes-deals-lidl-source-sync-dispatch'
CONFIG='/etc/hermes-deals-audits.d/lidl-source-sync.conf'
SUDOERS='/etc/sudoers.d/hermes-deals-lidl-source-sync'
...
TMP="$(mktemp -d /tmp/hermes-deals-lidl-source-sync-install.XXXXXX)"; KEEP_TMP=true
cleanup(){ if [[ "$KEEP_TMP" == false ]]; then rm -rf -- "$TMP"; else printf 'INSTALL_STAGING_PRESERVED=%s\n' "$TMP" >&2; fi; }
trap cleanup EXIT
cat > "$TMP/config" <<EOF_CONFIG
bridge_contract_version='hermes-deals-lidl-source-sync-v1'
...
github-runner ALL=(root) NOPASSWD: /usr/local/sbin/hermes-deals-lidl-source-sync-dispatch *
EOF_SUDOERS
chmod 0600 "$TMP/config" "$TMP/sudoers"
visudo -cf "$TMP/sudoers" >/dev/null || fail 'source-sync sudoers validation failed'
# Bootstrap registration only. It never fetches or runs the dispatcher.
