...
FINALIZER_VERSION='netto-card-region-topology-audit-owner-finalizer-v1'
...
INSTALLER_REL='tools/runner/install-netto-card-region-topology-rpi5-audit.sh'
...
[[ $# -eq 0 ]] || fail 'usage: run-netto-card-region-topology-audit-owner-finalizer.sh'
[[ "$(id -un)" == andris ]] || fail 'owner finalizer must run as andris'
for command in awk cat date gh git grep id install mktemp python3 readlink runuser sha256sum stat sudo tee tr visudo; do
  command -v "$command" >/dev/null 2>&1 || fail "required command is missing: $command"
done

stamp="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="/home/andris/netto-card-region-topology-audit-owner-finalizer-${stamp}.log"
INSTALL_LOG="$(mktemp /tmp/netto-card-region-topology-audit-install.XXXXXX)"
SUDO_LIST_LOG="$(mktemp /tmp/netto-card-region-topology-audit-sudo-list.XXXXXX)"
exec > >(tee -a "$LOG") 2>&1
trap on_error ERR
cleanup() {
...
git_read() {
  ...
  out="$(mktemp /tmp/netto-card-region-topology-git-out.XXXXXX)"
  err="$(mktemp /tmp/netto-card-region-topology-git-err.XXXXXX)"
  set +e
...
printf '=== Netto #305 card-region topology audit owner finalizer ===\n'
printf 'UTC=%s\nFINALIZER_VERSION=%s\nSOURCE_PR=%s\nTARGET_SHA=%s\nLOG=%s\n' \
...
if [[ -e "$WORKTREE" ]]; then
  [[ -d "$WORKTREE" && ! -L "$WORKTREE" ]] || fail 'topology audit worktree path is unsafe'
else
  install -d -m 0755 "$(dirname "$WORKTREE")"
  git -C "$PRIMARY" worktree add --detach "$WORKTREE" "$TARGET_SHA"
fi

[[ "$(git_read "$WORKTREE" rev-parse HEAD)" == "$TARGET_SHA" ]] || fail 'topology audit worktree HEAD mismatch'
[[ -z "$(git_read "$WORKTREE" branch --show-current)" ]] || fail 'topology audit worktree must be detached'
[[ -z "$(git_read "$WORKTREE" status --porcelain=v1 --untracked-files=all)" ]] || fail 'topology audit worktree is dirty'
[[ "$(stat -c '%U:%G' "$WORKTREE")" == andris:andris ]] || fail 'topology audit worktree ownership mismatch'
...
[[ "$COMMON_DIR" == '/home/andris/hermes-deals/.git' ]] || fail 'topology audit source is not a primary-repository worktree'
...
case "$origin" in
  ...
  *) fail 'topology audit worktree origin is not allowlisted' ;;
esac
verify_primary_unchanged

INSTALLER="$WORKTREE/$INSTALLER_REL"
[[ -f "$INSTALLER" && ! -L "$INSTALLER" ]] || fail 'reviewed topology audit installer is missing or unsafe'
set +e
sudo bash "$INSTALLER" "$TARGET_SHA" "$WORKTREE" 2>&1 | tee "$INSTALL_LOG"
INSTALL_RC=${PIPESTATUS[0]}
set -e
[[ "$INSTALL_RC" -eq 0 ]] || fail 'topology audit root trust installer failed'
...
[[ -f "$CONFIG" && ! -L "$CONFIG" ]] || fail 'installed topology audit config is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$CONFIG")" == 'root:root:644' ]] || fail 'installed topology audit config metadata mismatch'
[[ -f "$SUDOERS" && ! -L "$SUDOERS" ]] || fail 'installed topology audit sudoers is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$SUDOERS")" == 'root:root:440' ]] || fail 'installed topology audit sudoers metadata mismatch'
sudo visudo -cf "$SUDOERS" >/dev/null

[[ "$(config_value audit_name)" == 'netto-card-region-topology-audit-v1' ]] || fail 'registered audit name mismatch'
[[ "$(config_value commit_sha)" == "$TARGET_SHA" ]] || fail 'registered topology commit mismatch'
...
grep -Fq "$DISPATCHER" "$SUDO_LIST_LOG" || fail 'runner topology-audit dispatcher authorization is missing'
...
printf 'NEXT_GITHUB_ACTION=apply audit:netto-card-region-topology-v1 to merged PR #379\n'
