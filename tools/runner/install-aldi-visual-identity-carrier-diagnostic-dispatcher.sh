...
[[ $# -eq 1 ]] || fail "usage: sudo bash tools/runner/install-aldi-visual-identity-carrier-diagnostic-dispatcher.sh <merged-commit-sha>"
EXPECTED_SHA="$1"
...
LIBEXEC='/usr/local/libexec/hermes-deals-audits/aldi-visual-identity-carrier-diagnostic-v01'
...
SUDOERS='/etc/sudoers.d/hermes-deals-aldi-visual-identity-carrier-diagnostic'

declare -A source_files=(
  [diagnostic]='tools/aldi_visual_identity_carrier_diagnostic.py'
  ...
  [dispatcher]='tools/runner/aldi-visual-identity-carrier-diagnostic-dispatcher.sh'
)

for user in andris github-runner; do
  id "$user" >/dev/null 2>&1 || fail "required user missing: $user"
done
for command in awk bash git grep id install mktemp mv python3 readlink rm runuser sha256sum stat sudo systemctl visudo; do
...
tmp="$(mktemp -d /tmp/hermes-aldi-visual-identity-carrier-diagnostic-install.XXXXXX)"
cleanup() { rm -rf -- "$tmp"; }
trap cleanup EXIT

for key in diagnostic producer gate_a gate_b gate_c bridge two_cycle dispatcher; do
  git_read show "$EXPECTED_SHA:${source_files[$key]}" > "$tmp/$key"
  [[ -s "$tmp/$key" ]] || fail "empty registered source: $key"
done
python3 -m py_compile "$tmp/diagnostic" "$tmp/producer" "$tmp/gate_a" "$tmp/gate_b" "$tmp/gate_c" "$tmp/bridge" "$tmp/two_cycle"
bash -n "$tmp/dispatcher"

cat > "$tmp/sudoers" <<'SUDOERS'
Defaults!/usr/local/sbin/hermes-deals-aldi-visual-identity-carrier-diagnostic env_reset,secure_path=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
github-runner ALL=(root) NOPASSWD: /usr/local/sbin/hermes-deals-aldi-visual-identity-carrier-diagnostic
SUDOERS
...
install -o root -g root -m 0555 "$tmp/diagnostic" "$LIBEXEC/aldi_visual_identity_carrier_diagnostic.py"
install -o root -g root -m 0444 "$tmp/producer" "$LIBEXEC/aldi_new_baseline_weekly_shadow_producer.py"
...
diagnostic_sha="$(sha256sum "$LIBEXEC/aldi_visual_identity_carrier_diagnostic.py" | awk '{print $1}')"
...
conf_tmp="$(mktemp /etc/hermes-deals-audits.d/.aldi-visual-identity-carrier-diagnostic.conf.XXXXXX)"
cat > "$conf_tmp" <<CONF
registered_main_sha='$EXPECTED_SHA'
diagnostic_path='$LIBEXEC/aldi_visual_identity_carrier_diagnostic.py'
...
sudo -l -U github-runner | grep -Fq "$DISPATCHER" || fail "github-runner diagnostic sudo rule missing"
