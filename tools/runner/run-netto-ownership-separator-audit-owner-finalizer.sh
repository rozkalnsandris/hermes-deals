...
FINALIZER_VERSION='netto-ownership-separator-audit-owner-finalizer-v1'
...
INSTALLER_REL='tools/runner/install-netto-ownership-separator-rpi5-audit.sh'
...
[[ $# -eq 0 ]] || fail 'usage: run-netto-ownership-separator-audit-owner-finalizer.sh'
...
LOG="/home/andris/netto-ownership-separator-audit-owner-finalizer-${stamp}.log"
INSTALL_LOG="$(mktemp /tmp/netto-ownership-audit-install.XXXXXX)"
SUDO_LIST_LOG="$(mktemp /tmp/netto-ownership-audit-sudo-list.XXXXXX)"
exec > >(tee -a "$LOG") 2>&1
trap on_error ERR
cleanup() {
...
git_read() {
  ...
  out="$(mktemp /tmp/netto-ownership-audit-git-out.XXXXXX)"
  err="$(mktemp /tmp/netto-ownership-audit-git-err.XXXXXX)"
  set +e
...
printf '=== Netto #305 ownership audit owner finalizer ===\n'
printf 'UTC=%s\nFINALIZER_VERSION=%s\nSOURCE_PR=%s\nTARGET_SHA=%s\nLOG=%s\n' \
...
if [[ -e "$WORKTREE" ]]; then
  [[ -d "$WORKTREE" && ! -L "$WORKTREE" ]] || fail 'ownership audit worktree path is unsafe'
else
  install -d -m 0755 "$(dirname "$WORKTREE")"
  git -C "$PRIMARY" worktree add --detach "$WORKTREE" "$TARGET_SHA"
fi

[[ "$(git_read "$WORKTREE" rev-parse HEAD)" == "$TARGET_SHA" ]] || fail 'ownership audit worktree HEAD mismatch'
[[ -z "$(git_read "$WORKTREE" branch --show-current)" ]] || fail 'ownership audit worktree must be detached'
[[ -z "$(git_read "$WORKTREE" status --porcelain=v1 --untracked-files=all)" ]] || fail 'ownership audit worktree is dirty'
[[ "$(stat -c '%U:%G' "$WORKTREE")" == andris:andris ]] || fail 'ownership audit worktree ownership mismatch'
...
[[ "$COMMON_DIR" == '/home/andris/hermes-deals/.git' ]] || fail 'ownership audit source is not a primary-repository worktree'
...
case "$origin" in
  ...
  *) fail 'ownership audit worktree origin is not allowlisted' ;;
esac
verify_primary_unchanged

INSTALLER="$WORKTREE/$INSTALLER_REL"
[[ -f "$INSTALLER" && ! -L "$INSTALLER" ]] || fail 'reviewed ownership audit installer is missing or unsafe'
set +e
sudo bash "$INSTALLER" "$TARGET_SHA" "$WORKTREE" 2>&1 | tee "$INSTALL_LOG"
INSTALL_RC=${PIPESTATUS[0]}
set -e
[[ "$INSTALL_RC" -eq 0 ]] || fail 'ownership audit root trust installer failed'
...
for marker in \
  ...
  'PRODUCTION_APPLY_AUTHORIZED=false'; do
...
[[ -f "$CONFIG" && ! -L "$CONFIG" ]] || fail 'installed audit config is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$CONFIG")" == 'root:root:644' ]] || fail 'installed audit config metadata mismatch'
[[ -f "$SUDOERS" && ! -L "$SUDOERS" ]] || fail 'installed audit sudoers is missing or unsafe'
[[ "$(stat -c '%U:%G:%a' "$SUDOERS")" == 'root:root:440' ]] || fail 'installed audit sudoers metadata mismatch'
sudo visudo -cf "$SUDOERS" >/dev/null

sudo -l -U github-runner > "$SUDO_LIST_LOG"
grep -Fq "$DISPATCHER" "$SUDO_LIST_LOG" || fail 'runner ownership-audit dispatcher authorization is missing'
...
printf 'NEXT_GITHUB_ACTION=apply audit:netto-ownership-separator-v1 to merged PR #330\n'
