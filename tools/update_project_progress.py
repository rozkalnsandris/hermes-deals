...
def render_readme_block(snapshot: Mapping[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "## Project progress",
        "",
        f"**Overall:** **{format_percent_tenths(overall)}%** `{render_progress_bar(overall)}`",
        "",
        f"**Progress during {previous_day}:** **+{format_project_units(delta)} percentage points** **({format_percent_tenths(previous)}% → {format_percent_tenths(overall)}%)**",
        "",
        "**Store catalogues**",
        *stores,
        "",
        ...
        "",
        f"**Issues fixed:** **{snapshot['completed_issue_count']} total** · **{snapshot['previous_day_completed_issue_count']} during the previous day** — {links}",
        "",
