...
STRATEGY = "netto_weekly_github_live_source_v1"


class WeeklyLiveSourceError(ValueError):
    pass
...
def load_family_source(repo: Path) -> SourceConfig:
    ...
    if path.is_symlink() or not path.is_file():
        raise WeeklyLiveSourceError("authoritative sources.json is missing or unsafe")
    matches = [
        source
...
def select_latest_nonexpired(
    ...
    if not eligible:
        raise WeeklyLiveSourceError("no non-expired Netto prospect was fetched")
...
def fetch_latest_nonexpired(source: SourceConfig, *, as_of: date) -> NettoStoreProspectBundle:
    if source.store_external_id != STORE_ID or source.scope != SCOPE:
        raise WeeklyLiveSourceError("live source is not bound to family Netto 5659")
    ...
    with httpx.Client(follow_redirects=True, timeout=45, headers=headers) as client:
        ...
        if not any(
            ...
            raise WeeklyLiveSourceError("Netto selected-store cookie missing")
        viewers = extract_netto_direct_viewers(store.text, STORE_ID)
        if not viewers:
            raise WeeklyLiveSourceError("Netto store page exposes no digital weekly prospects")
        for slug, url in sorted(viewers.items()):
            try:
                ...
                if html_validity is not None and html_validity[:2] != pdf_validity[:2]:
                    raise WeeklyLiveSourceError("Netto prospect HTML/PDF validity mismatch")
                ...
                bundles.append(
                    NettoStoreProspectBundle(
                        ...
                        validity_source_type=(
                            "prospect_html_meta" if html_validity else "prospect_pdf_text"
                        ),
    ...
    try:
        ...
        raise WeeklyLiveSourceError(f"{exc}; failures={failures}") from exc
...
def materialize(repo: Path, raw_root: Path, as_of: date, output: Path) -> dict[str, object]:
    ...
    if raw_root.exists() or raw_root.is_symlink():
        raise WeeklyLiveSourceError("raw output root must be create-only")
    if output.exists() or output.is_symlink():
        raise WeeklyLiveSourceError("live-source summary must be create-only")

    source = load_family_source(repo)
    bundle = fetch_latest_nonexpired(source, as_of=as_of)
    if bundle.valid_until < as_of:
        raise WeeklyLiveSourceError("selected weekly source is expired")
    ...
    if raw_root not in manifest.resolve().parents:
        raise WeeklyLiveSourceError("immutable manifest escaped the temporary raw root")
    ...
    payload: dict[str, object] = {
        ...
        "pdf_sha256": sha256(bundle.prospect_pdf).hexdigest(),
        "network_fetch_performed": True,
        "family_store_scope_verified": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch the latest non-expired family Netto 5659 weekly source without held-out identity pinning."
    )
