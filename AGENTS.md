...
- Before Web presentation, UI architecture, client-state, or realtime-transport work, read `docs/WEB_ARCHITECTURE.md` and `.github/web-architecture-v1.json`; they are the canonical Web contract.
- The canonical Web target is `FastAPI + PostgreSQL + Jinja + HTMX + semantic HTML + plain CSS + minimal Vanilla JS + SSE + Cloudflare Access/Tunnel`. Existing Vanilla-JS/CSS UI and deterministic Node build tooling are migration implementation details, not authority to introduce a SPA, offline application state, a Node production application runtime, or WebSocket-default architecture. Changing those boundaries requires an explicit owner architecture decision and synchronized canonical-contract updates.
...
- `START hermes-deals` uses the repository-local startup routing. Bootstrap only enough state to identify one current work item/lane/gate: current `AGENTS.md`/rules, canonical handoff or continuation when present, current default-branch SHA, and only the issue/PR state required by that lane.
...
- `SYNC hermes-deals` is incremental refresh of the current lane, not a repo-wide audit. Re-read a handoff only when continuation may have changed or is ambiguous.
...
- When no owner gate exists and mutable GitHub/external state must be refreshed, output `SYNC hermes-deals`.
- When no owner gate exists and same-scope safe technical continuation is immediately available, output `turpini`.
- When the current outcome is complete and no same-scope continuation remains, output `START hermes-deals`.
