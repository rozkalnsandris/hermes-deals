...
def test_installer_grants_only_regex_bound_fixed_dispatcher_sudo_boundary() -> None:
    ...
    assert "host sudoers parser lacks required regex support" in text
    ...
    assert 'runuser -u github-runner -- sudo -n -l' not in text
    assert "ALL=(ALL)" not in text


def test_sudo_policy_probes_allow_exact_shape_and_deny_malformed_arguments() -> None:
    text = _text()
    assert 'sudo_policy_must_deny()' in text
    for marker in (
