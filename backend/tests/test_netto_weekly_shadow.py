...
class EvidenceBindingTest(unittest.TestCase):
    ...
    def test_verified_no_pdf_is_explicit_not_missing(self):
        binding = _binding(
            pdf_status="verified_no_pdf",
            pdf_path=None,
            pdf_sha256=None,
            no_pdf_reason="official viewer exposes no PDF for this campaign",
        )
        binding.validate()
        self.assertEqual(binding.pdf_status, MODULE.EvidenceStatus.VERIFIED_NO_PDF)
...
class WeeklyStateMachineTest(unittest.TestCase):
    def _input(self, **overrides):
        payload = {
            ...
            "previous_manifest_sha256": _sha("d"),
            "shadow_passed": None,
            "retry_count": 0,
    ...
    def test_missing_evidence_retries_then_alerts(self):
        payload = self._input()
        payload["binding"]["pdf_status"] = "missing"
