...
class UiReferenceRebuildV10bTest(unittest.TestCase):
    ...
    def test_archived_v9b_replacement_markers_are_not_shipped(self):
        ...
        self.assertEqual(
            self.html.count('new URLSearchParams({as_of:iso,view:"current",sort:"discount_desc",limit:"500",offset:"0"})'),
            0,
        )
        self.assertEqual(
            self.html.count("Promise.all([fetchJson(dailySpecialsUrl(today)),fetchJson(dailySpecialsUrl(tomorrow))])"),
            0,
        )
