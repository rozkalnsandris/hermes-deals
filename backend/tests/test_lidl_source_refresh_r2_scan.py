...
def test_r2_review_is_exact_existing_contract() -> None:
    review = r2.build_approved_source_review()
...
def test_r2_authorization_is_bound_to_r1_evidence() -> None:
    assert r2.AUTHORIZATION_COMMENT_ID == 5225820964
    assert r2.EXPECTED_AS_OF == "2026-08-08"
    assert r2.EXPECTED_LIVE_RAW_SHA256 == (
...
def test_r2_rejects_wrong_authorization_before_network() -> None:
    with tempfile.TemporaryDirectory() as raw:
        with pytest.raises(r2.R2ScanError, match="authorization comment ID mismatch"):
            r2.run_r2(
                as_of="2026-08-08",
...
def test_public_issue_comment_workflow_is_hosted_and_owner_gated() -> None:
    ...
    assert "AUTHORIZATION_COMMENT_ID: \"5225820964\"" in text
    assert "/hermes-lidl-source-refresh-r2 as_of=2026-08-08" in text
