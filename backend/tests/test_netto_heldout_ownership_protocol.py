...
SHA_D = "d" * 64
SHA_E = "e" * 64
PREDICTION_PARSER = "netto-visual-geometry-shadow-v3-unrotated-page-space"
SOURCE_PARSER = "netto-store-prospect-source-v1"
...
def manifest() -> dict[str, object]:
    return {
        ...
        "source_sha256": SHA_A,
        "parser_identity": PREDICTION_PARSER,
...
def test_prepare_freeze_derives_campaign_from_source_and_separates_parser_identities(tmp_path: Path) -> None:
    ...
    assert frozen["parser_identity"] == PREDICTION_PARSER
    assert frozen["parser_identity"] != binding["parser_identity"]
...
def test_prepare_freeze_requires_prediction_parser_identity(tmp_path: Path) -> None:
