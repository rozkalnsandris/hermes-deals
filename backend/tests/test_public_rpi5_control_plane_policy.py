...
def test_public_rpi5_inventory_covers_every_self_hosted_workflow() -> None:
    ...
    assert len(declared) == len(workflows) == 55
    ...
    assert len(all_workflows) == policy["inventory_counts"]["workflow_files"] == 89
    assert len(actual) == policy["inventory_counts"]["self_hosted_workflows"] == 55
