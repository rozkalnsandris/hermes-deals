...
class UiReferenceRebuildV10cTest(unittest.TestCase):
    ...
    def test_release_marker_is_present(self):
        ...
        self.assertIn("reference-v10c-available-count-pagination", self.html)
    ...
    def test_daily_special_loader_pages_through_available_count(self):
        ...
        self.assertIn(
            "Piedāvājumu skaits mainījās lapošanas laikā",
            self.html,
        )
    ...
    def test_active_loader_uses_real_current_deals_response_contract(self):
        ...
        self.assertNotIn(
            "if(total==null)total=Number(payload.total??rows.length)",
            without_comments,
        )
        self.assertIn(
            "API neatgrieza derīgu kopējo piedāvājumu skaitu",
            without_comments,
        )
