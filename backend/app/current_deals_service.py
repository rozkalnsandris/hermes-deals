...
from app.completeness_rescue_read import dedupe_completeness_rescue_publications
...
StateRowLoader = Callable[[Session, date], list[tuple[str, OfferCandidateRecord]]]
StageRecorder = Callable[[str, float], None]


def _noop_stage_recorder(_name: str, _started: float) -> None:
    return None
...
def load_python_ranked_state_rows(
    db: Session,
    effective_date: date,
...
def build_current_deals(
    *,
    ...
    record_stage: StageRecorder = _noop_stage_recorder,
) -> CurrentDealsOut:
    rank_started = perf_counter()
    visible_state_rows = state_row_loader(db, effective_date)
    record_stage("rank", rank_started)
    ...
    record_stage("filter-sort", filter_started)
    ...
    record_stage("canonical", canonical_started)
    ...
    record_stage("model", model_started)
    return payload
